...
export default angular.module('xo', [
  ...
  .service('xo', function (xoApi, xoNotify) {
    ...
    const xo = {
      ...
      mount: {
        getAll: action('Get all nfs mount points', 'mount.getAll'),
        create: action('Create a nfs mount point', 'mount.create', {
          argsMapper: (path, host, share) => ({path, host, share})
        }),
        set: action('Modify a nfs mount point', 'mount.set', {
          argsMapper: (id, path = undefined, host = undefined, share = undefined, mounted = undefined) => {
            const args = {id}
            path !== undefined && (args.path = path)
            host !== undefined && (args.host = host)
            share !== undefined && (args.share = share)
            mounted !== undefined && (args.mounted = mounted)
            return args
          }
        }),
        delete: action('Delete a nfs mount point', 'mount.delete', {
          argsMapper: (id) => ({id})
        }),
        getBackupMountPath: action('Get configured back-up root path', 'mount.getBackupMountPath')
      }
    }
